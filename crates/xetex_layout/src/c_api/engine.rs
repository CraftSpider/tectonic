...
use crate::engine::{find_graphite_feature_named, LayoutEngine};
use std::borrow::Cow;
...
pub unsafe extern "C" fn getGlyphHeightDepth(
    ...
    let (h, d) = (*engine).font_mut().get_glyph_height_depth(glyph_id as u16);
    if !height.is_null() {
        *height = h;
    }
    if !depth.is_null() {
...
pub unsafe extern "C" fn mapCharToGlyph(engine: XeTeXLayoutEngine, char_code: u32) -> u32 {
    (*engine)
        .font()
        .map_char_to_glyph(char::from_u32(char_code).unwrap()) as u32
}
...
pub unsafe extern "C" fn initGraphiteBreaking(
    ...
    let text = slice::from_raw_parts(txt_ptr, txt_len as usize);
    (*engine).init_graphite_break(text)
}
